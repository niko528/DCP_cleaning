...
class DPC(object):
    ...
    def __init__(self, path, data_name, n=0, dc_method=0, dc_percent=1, rho_method=1, delta_method=1, use_halo=False, plot=None):
        ...
        # pd.DataFrame(rho).to_csv('rho702best1_precleaning.csv')
        # 计算 delta
        delta = self.get_delta(d_matrix, max_id, rho, delta_method)
        # pd.DataFrame(delta).to_csv('delta702best1_precleaning.csv')
        # 确定聚类中心
        center, gamma = self.get_center(d_matrix, rho, delta, dc, n, max_id)
        # pd.DataFrame(gamma).to_csv('gamma702best1_precleaning.csv')
        # pd.DataFrame(center).to_csv('center702best1_precleaning.csv')
        # 聚类
        cluster = self.assign(d_matrix, dc, rho, delta, center, max_id)
        # cluster_dict = open('cluster_dict.xls', 'w')   #把聚类结果写进一个表格查看
        # for key in cluster:
        #     cluster_dict.write(str(key))
        #     cluster_dict.write('\t')
        #     cluster_dict.write('\t'.join(str(cluster[key]).split(",")))
        #     cluster_dict.write('\n')
        # cluster_dict.close()
        # s = str(cluster)   #把聚类结果写进一个文档查看
        ...
        # pd.DataFrame(halo).to_csv('halo702best1_precleaning.csv')
        if plot is None:
            # 单数据集绘制图分布
            ...
            # self.draw_roh_delta(rho, delta, center, axes[0])
            # self.draw_gamma(rho, delta, axes[1])
            self.draw_cluster(data_name, cluster, halo, points, axes)
            plt.show()
        else:
            # 全部数据集画图
            self.draw_cluster(data_name, cluster, halo, points, plot)
    ...
    def get_halo(self, d, rho, cluster, center, dc, max_id):
        ...
        # pd.DataFrame(cluster).to_csv('cluster.csv')
        # pd.DataFrame(halo).to_csv('halo.csv')
        return cluster, halo
    ...
    def draw_cluster(self, title, cluster, halo, points, plot):
        ...
        plt.savefig("cluster_702_hole", dpi=960)
        # plot.set_title(title)


# 进程函数
# path | title | N: 聚类数 | dc method | dc per | rho method | delta method | use_halo | plot
def cluster(path, data_name, n, dc_method=0, dc_percent=1, rho_method=1, delta_method=1, use_halo=True, plot=None):
...
if __name__ == '__main__':
    ...
    # 绘制指定数据集详情
    p = Pool(4)
    path = sys.path[0] + '/dataset/myself/'
    #  path | title | N: 聚类数 | dc method | dc per | rho method | delta method | use_halo | plot
    ...
    p.apply_async(cluster, args=(path + 'F7best1_DPCdata.dat', 'DPCdata', 7, 0, 2))
    p.close()
    p.join()

    # DPC(path + 'outtest.dat', 'outtest', 15)